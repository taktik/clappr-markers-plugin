...
/***/ (function(module, exports, __webpack_require__) {
	...
	'use strict';Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();var _clappr=__webpack_require__(1);__webpack_require__(27);var _marker=__webpack_require__(4);var _marker2=_interopRequireDefault(_marker);var _standardMarker=__webpack_require__(24);var _standardMarker2=_interopRequireDefault(_standardMarker);var _cropMarker=__webpack_require__(22);var _cropMarker2=_interopRequireDefault(_cropMarker);var _imageMarker=__webpack_require__(23);var _imageMarker2=_interopRequireDefault(_imageMarker);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}function _possibleConstructorReturn(self,call){if(!self){throw new ReferenceError("this hasn't been initialised - super() hasn't been called");}return call&&(typeof call==="object"||typeof call==="function")?call:self;}function _inherits(subClass,superClass){if(typeof superClass!=="function"&&superClass!==null){throw new TypeError("Super expression must either be null or a function, not "+typeof superClass);}subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:false,writable:true,configurable:true}});if(superClass)Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass;}var MarkersPlugin=function(_UICorePlugin){_inherits(MarkersPlugin,_UICorePlugin);_createClass(MarkersPlugin,[{key:'name',get:function get(){return'markers-plugin';}},{key:'attributes',get:function get(){return{'class':this.name};}}],[{key:'Marker',get:function get(){return _marker2.default;}},{key:'StandardMarker',get:function get(){return _standardMarker2.default;}},{key:'CropMarker',get:function get(){return _cropMarker2.default;}},{key:'ImageMarker',get:function get(){return _imageMarker2.default;}// backwards compatibility
	   ...
	   * Add a new marker.
	   */},{key:'addMarker',value:function addMarker(marker){var internalMarker=this._buildInternalMarker(marker);marker.videoDuration=this._duration;this._markers.push(internalMarker);this._createMarkerEl(internalMarker);this._renderMarkers();}/*
	...
	},{key:'_buildInternalMarker',value:function _buildInternalMarker(marker){var $tooltip=marker.getTooltipEl();if($tooltip){$tooltip=(0,_clappr.$)($tooltip);}return{source:marker,emitter:marker.getEmitter(),$marker:(0,_clappr.$)(marker.getMarkerEl()),markerLeft:null,$tooltip:$tooltip,$tooltipContainer:null,tooltipContainerLeft:null,tooltipContainerBottom:null,tooltipChangedHandler:null,time:marker.getTime(),duration:marker.getDuration?marker.getDuration():undefined,timeChangedHandler:null,onDestroy:marker.onDestroy};}},{key:'_createInitialMarkers',value:function _createInitialMarkers(){var _this2=this;var markers=this._getOptions().markers;if(!markers){return;}this._markers=[];markers.forEach(function(a){_this2._markers.push(_this2._buildInternalMarker(a));});// append the marker elements to the dom
	...
	_this3._updateMarkerTime(marker);};marker.emitter.on("timeChanged",marker.timeChangedHandler);$marker.click(function(e){console.log('$marker.click');// when marker clicked seek to the exact time represented by the marker
...
/***/ (function(module, exports, __webpack_require__) {
	 ...
	 */var Marker=function(){function Marker(){_classCallCheck(this,Marker);this._emitter=new _eventEmitter2.default({});this.initAttributes();}/*
	...
	}},{key:"initAttributes",value:function initAttributes(){this.videoDuration=0;}}]);return Marker;}();exports.default=Marker;module.exports=exports["default"];
...
/***/ (function(module, exports, __webpack_require__) {
	 ...
	 */var CropMarker=function(_BaseMarker){_inherits(CropMarker,_BaseMarker);/*
	   ...
	   */function CropMarker(time,duration){_classCallCheck(this,CropMarker);var _this=_possibleConstructorReturn(this,(CropMarker.__proto__||Object.getPrototypeOf(CropMarker)).call(this,time));_this._duration=duration||5;return _this;}/*
	   * Returns the duration (in seconds) that this marker represents.
	   */_createClass(CropMarker,[{key:'getDuration',value:function getDuration(){var duration=this._updateDurationValueFromCss();console.log(duration);return duration;}},{key:'_updateDurationValueFromCss',value:function _updateDurationValueFromCss(){var duration=this.videoDuration/100*parseFloat(this.getMarkerEl()[0].style.width);if(isNaN(duration)){duration=this._duration;}else{this._duration=duration;}return duration;}/*
	   * Set the duration (in seconds) that this marker should represents.
	   */},{key:'setDuration',value:function setDuration(duration){// TODO manager 0
	if(this._duration!==duration){debugger;this._renderDuration(duration);this._duration=duration;}}},{key:'_renderDuration',value:function _renderDuration(duration){var percentage=Math.min(Math.max(this._time/this.videoDuration*100,0),100);var percentageWidth=Math.min(Math.max(duration/this.videoDuration*100,0),100-percentage);this.getMarkerEl().css("width",percentageWidth+"%");//console.log("Width:" + percentageWidth + "%")
	}},{key:'_renderTime',value:function _renderTime(time){var percentage=Math.min(Math.max(time/this.videoDuration*100,0),100);this.getMarkerEl().css("left",percentage+"%");//console.log("Left:" + percentage + "%")
	}},{key:'render',value:function render(){//console.log('render')
	this._renderDuration(this._duration);this._renderTime(this._time);}/*
		 * Should return the time (in seconds) that this marker represents.
		 */},{key:'getTime',value:function getTime(){var time=this._updateTimeFromCss();console.log(time);return time;}},{key:'_updateTimeFromCss',value:function _updateTimeFromCss(){var time=this.videoDuration/100*parseFloat(this.getMarkerEl()[0].style.left);if(isNaN(time)){time=this._time;}else{this._time=time;}return time;}/*
		 * Set the time (in seconds) that this marker represents.
		 */},{key:'setTime',value:function setTime(time){if(this._time!==time){this._renderTime(time);this._time=time;}}},{key:'_buildMarkerEl',value:function _buildMarkerEl(){var _this2=this;var $marker=(0,_clappr.$)('<div />').addClass('crop-marker');$marker.append((0,_clappr.$)('<div id="wgrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-w').addClass('left'));$marker.append((0,_clappr.$)('<div id="egrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-e').addClass('right'));jQuery($marker[0]).resizable({handles:{'w':'#wgrip','e':'#egrip'},resize:function resize(event,ui){console.log('resize');ui.originalElement.css({'left':''+1.0*ui.position.left/ui.originalElement.parents('.bar-container').width()*100.0+'%','width':''+1.0*ui.size.width/ui.originalElement.parents('.bar-container').width()*100.0+'%'});_this2._updateDurationValueFromCss();_this2._updateTimeFromCss();},maxHeight:20,minHeight:20});return $marker;}}]);return CropMarker;}(_baseMarker2.default);exports.default=CropMarker;module.exports=exports['default'];
