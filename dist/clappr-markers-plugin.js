...
/***/ (function(module, exports, __webpack_require__) {
	 ...
	 */var CropMarker=function(_BaseMarker){_inherits(CropMarker,_BaseMarker);/*
	   ...
	   */_createClass(CropMarker,[{key:'getDuration',value:function getDuration(){var duration=this._updateDurationValueFromCss();return duration;}},{key:'_updateDurationValueFromCss',value:function _updateDurationValueFromCss(){var duration=this.videoDuration/100*parseFloat(this.getMarkerEl()[0].style.width);if(isNaN(duration)){duration=this._duration;}else{this._duration=duration;}return duration;}/*
	...
	this._renderDuration(this._duration);this._renderTime(this._time);}/*
		 * Should return the time (in seconds) that this marker represents.
		 */},{key:'getTime',value:function getTime(){var time=this._updateTimeFromCss();return time;}},{key:'_updateTimeFromCss',value:function _updateTimeFromCss(){var time=this.videoDuration/100*parseFloat(this.getMarkerEl()[0].style.left);if(isNaN(time)){time=this._time;}else{this._time=time;}return time;}/*
		 * Set the time (in seconds) that this marker represents.
		 */},{key:'setTime',value:function setTime(time){if(this._time!==time){this._renderTime(time);this._time=time;}}},{key:'_buildMarkerEl',value:function _buildMarkerEl(){var _this2=this;var $marker=(0,_clappr.$)('<div />').addClass('crop-marker');$marker.append((0,_clappr.$)('<div id="wgrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-w').addClass('left'));$marker.append((0,_clappr.$)('<div id="egrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-e').addClass('right'));jQuery($marker[0]).resizable({handles:{'w':'#wgrip','e':'#egrip'},resize:function resize(event,ui){ui.originalElement.css({'left':''+1.0*ui.position.left/ui.originalElement.parents('.bar-container').width()*100.0+'%','width':''+1.0*ui.size.width/ui.originalElement.parents('.bar-container').width()*100.0+'%'});_this2._updateDurationValueFromCss();_this2._updateTimeFromCss();},maxHeight:20,minHeight:20});return $marker;}}]);return CropMarker;}(_baseMarker2.default);exports.default=CropMarker;module.exports=exports['default'];
